# Ежеминутка

Публичный сайт единой сети тайм-кафе с ёжиками. Это **один бренд и одно Next.js-приложение**, а Москва, Санкт-Петербург и Казань представлены как три независимых `location`.

## Мультигородская структура

- общая страница сети: `/`;
- городские лендинги: `/moscow`, `/spb`, `/kazan`;
- городские разделы: `/{city}/gallery`, `/{city}/news`, `/{city}/booking`;
- общие страницы выбора города: `/gallery`, `/news`, `/booking`;
- общая политика: `/privacy`.

Динамические маршруты используют единый набор компонентов. Не создавайте отдельные приложения или копии JSX для городов. Будущие сущности backend должны связываться с location (`location_id`).

## Где обновлять данные

- `src/config/locations.ts` — адреса, телефоны, email, часы и другие данные кафе;
- `src/content/tariffs.ts` — отдельная гибкая тарифная модель каждого города;
- `src/content/services.ts` — доступность услуг по городам;
- `src/config/media.ts` — отдельные media-пути каждого города;
- `public/media/moscow/`, `public/media/spb/`, `public/media/kazan/` — фотографии кафе.

Пока media-значения равны `null`, интерфейс показывает placeholders. Рекомендуемые имена файлов перечислены в README каждой media-папки.

## Локальный запуск и production

Требуется Node.js 24.

```bash
npm install
npm run dev
```

Сайт будет доступен на `http://localhost:3000`. Production-проверка и запуск:

```bash
npm run build
npm start
```

Дополнительные проверки:

```bash
npm run typecheck
npm run format:check
```

## Бронирование и PostgreSQL

Бронирования хранятся в обычном PostgreSQL через `pg`, без ORM. Настройки задаются переменными окружения:

- `DATABASE_URL` — строка подключения к PostgreSQL;
- `BOOKING_ENABLED` — бронирование доступно только при точном значении `true`; безопасное значение по умолчанию — `false`.

Скопируйте `.env.example` в локальный `.env` и примените SQL-миграции:

```bash
npm run db:migrate
```

Runner выполняет файлы `db/migrations/*.sql` по имени и записывает применённые файлы в
`schema_migrations`. Приложение не подключается к базе при импорте модулей или во время сборки.
Проверка состояния доступна на `GET /api/health`.

Пока юридические реквизиты не заполнены и production PostgreSQL не настроен, необходимо держать
`BOOKING_ENABLED=false`. Значение `true` будет установлено только перед реальным запуском.

Последовательность запуска на Timeweb:

1. создать PostgreSQL;
2. задать `DATABASE_URL`;
3. выполнить `npm run db:migrate`;
4. проверить `/api/health`;
5. заполнить юридические данные оператора;
6. установить `BOOKING_ENABLED=true`.

На текущем Vercel без `DATABASE_URL` публичный сайт продолжает работать, а форма бронирования
остаётся видимой, но отключённой.

## Docker

```bash
docker build -t ezheminutka .
docker run --rm -p 3000:3000 ezheminutka
```

Контейнер слушает `0.0.0.0:3000`.

## Материалы владельца

Оригинальный логотип нужно вручную положить в `public/brand/logo-original.png`, затем заменить
`logoSrc` в `src/config/brand.ts` на `/brand/logo-original.png`.

Реальные фотографии также загружаются вручную в media-папку соответствующего города. После
загрузки пути указываются в `src/config/media.ts`; JSX менять не требуется.

## Админка

Единая административная система доступна по `/admin/login`; защищённая панель находится на
`/admin`. Роль `SUPERADMIN` управляет всеми кафе, а `LOCATION_ADMIN` — только явно назначенными
локациями. Права проверяются сервером при чтении и каждой мутации.

Порядок настройки: настройте PostgreSQL, выполните `npm run db:migrate`, задайте длинный случайный
`ADMIN_SESSION_SECRET` (рекомендуется не менее 32 случайных байт), создайте аккаунты и откройте
`/admin/login`. Учётные данные и пароли не хранятся в git или env-файлах репозитория. Пароль
запрашивается интерактивно и не принимается CLI-аргументом.

```bash
npm run admin:create -- --username admin-moscow --display-name "Москва" --role LOCATION_ADMIN --locations moscow
npm run admin:create -- --username admin-spb --display-name "Санкт-Петербург" --role LOCATION_ADMIN --locations spb
npm run admin:create -- --username admin-kazan --display-name "Казань" --role LOCATION_ADMIN --locations kazan
npm run admin:create -- --username superadmin --display-name "Администратор сети" --role SUPERADMIN
npm run admin:reset-password -- --username admin-moscow
```

Сброс пароля увеличивает `session_version` и завершает все прежние сессии пользователя.

## Управление внешним видом

Защищённая страница `/admin/appearance` позволяет менять логотип бренда (только
`SUPERADMIN`), первый экран города и карточки блока «Что есть в Ежеминутке» без
редактирования HTML/CSS. Администратор города работает только со своим кафе.

Изображения хранятся на локальном persistent filesystem, а метаданные — в
PostgreSQL (`media_assets`). Для production задайте:

```env
MEDIA_UPLOAD_ENABLED=true
UPLOAD_DIR=/app/uploads
```

`UPLOAD_DIR` должен быть постоянным mounted-каталогом, переживающим перезапуск
контейнера. На Vercel без постоянного диска оставьте загрузку отключённой; это не
мешает fallback-контенту и уже сохранённым URL отображаться. Production backup
обязательно должен включать **и PostgreSQL, и весь `UPLOAD_DIR`**: база содержит
метаданные, а бинарные файлы находятся на диске.

## Галерея

Защищённая страница `/admin/gallery` управляет фотографиями отдельно для каждого
кафе и использует существующие `media_assets` и `UPLOAD_DIR`. Для загрузки нужны
`UPLOAD_DIR` и `MEDIA_UPLOAD_ENABLED=true`.

## Demo админки

Интерактивная демонстрация доступна на `/admin/demo`. Она не требует БД или входа,
использует фиктивные данные и хранит изменения только в React state до обновления.
Локальные изображения остаются в браузере. Demo не является способом входа;
реальная защищённая админка по-прежнему доступна через `/admin/login`.
